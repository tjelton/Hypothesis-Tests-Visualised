...
# then open http://localhost:8000/t-test-2-sample/
```
...
Per the migration plan in `CLAUDE.md`: copy `t-test-2-sample/` to the
**`gh-pages` branch as a top-level dir**, alongside `shared/` and the other
lessons, served at
`https://tjelton.github.io/Hypothesis-Tests-Visualised/t-test-2-sample/`.
