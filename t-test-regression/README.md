...
# then open http://localhost:8000/t-test-regression/
```
...
Per the migration plan in `CLAUDE.md`: copy `t-test-regression/` to the
**`gh-pages` branch as a top-level dir**, alongside `shared/` and the other
lessons, served at
`https://tjelton.github.io/Hypothesis-Tests-Visualised/t-test-regression/`.
